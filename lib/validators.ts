...
export function validateFormSubmission(data: unknown): {
  ...
  if (!formData.teamRequirements || typeof formData.teamRequirements !== "object") {
    errors.push("Team requirements are required")
  } else {
    const tr = formData.teamRequirements as Record<string, unknown>
    if (!tr.teamSize) errors.push("Team size is required")
  ...
  const sanitizedData: FormSubmissionData = {
    teamRequirements: {
      teamSize: sanitizeString(tr.teamSize as string),
      currentSolution: sanitizeString(tr.currentSolution as string),
      painPoints: Array.isArray(tr.painPoints) ? tr.painPoints.map((p) => sanitizeString(String(p))).slice(0, 20) : [],
    },
    features: {
      features: Array.isArray(ft.features) ? ft.features.map((f) => sanitizeString(String(f))).slice(0, 50) : [],
    },
    timeLine: {
    ...
    security: {
      technicalRequirements: sanitizeString((sc.technicalRequirements as string) || ""),
